 ...
 * <p>
 * These serializers attempt to make the appropriate type conversions for numeric values.
 * <p>
 * In general, for any numeric type, in addition to the rules of {@link ScalarSerializer},
 *
 * <ul>
 *     <li>
 *         When serializing: <ul>
 *             <li>If any larger types are applicable for serialization, convert to one of those types</li>
 *             <li>Otherwise, write as a string</li>
 *         </ul>
 *     </li>
 *     <li>
 *         When deserializing decimal types ({@code float} and {@code double}): <ul>
 *             <li>If the input value is a {@link Number}, convert it to the applicable type, throwing an exception if it is out of bounds</li>
 *             <li>If the input value is a {@link CharSequence}, attempt to parse the numeric type, with an accepted
 *             suffix of {@code f} or {@code d}, aligned with the type of number being parsed</li>
 *         </ul>
 *     </li>
 *     <li>
 *         When deserializing whole number types ({@code byte}, {@code short}, {@code int}, and {@code long}): <ul>
 *             <li>If the input value is a {@link Float} or {@link Double}, is a whole number, and is within the bounds of the return type, return the casted value</li>
 *             <li>If the input value is a {@link Number}, convert it to the applicable type, throwing an exception if it is out of bounds</li>
 *             <li>If the input value is a {@link CharSequence}, attempt to parse the numeric type with the following affixes:
 *             <ul>
 ...
 *                 <li>An accepted suffix of {@code b}, {@code s}, {@code i}, or {@code l} (after {@code u} if present), aligned with the type of number being parsed</li>
 *             </ul>
 *             </li>
 *         </ul>
 *     </li>
...
class NumericSerializers {
    ...
    static final ScalarSerializer<Byte> BYTE = TypeSerializer.of(Byte.class, (v, pass) -> {
        ...
        if (value instanceof Number) {
            ...
            if (full > Byte.MAX_VALUE || full < Byte.MIN_VALUE) {
                throw new ObjectMappingException("Value " + full + " is out of range for a byte ([" + Byte.MIN_VALUE + "," + Byte.MAX_VALUE + "])");
            }
            return (byte) full;
        }
    ...
    static final ScalarSerializer<Short> SHORT = TypeSerializer.of(Short.class, (v, pass) -> {
        ...
        if (value instanceof Number) {
            ...
            if (full > Short.MAX_VALUE || full < Short.MIN_VALUE) {
                throw new ObjectMappingException("Value " + full + " is out of range for a short ([" + Short.MIN_VALUE + "," + Short.MAX_VALUE + "])");
            }
            return (short) full;
        }
    ...
    static final ScalarSerializer<Integer> INTEGER = TypeSerializer.of(Integer.class, (v, pass) -> {
        ...
        if (value instanceof Number) {
            ...
            if (full > Integer.MAX_VALUE || full < Integer.MIN_VALUE) {
                throw new ObjectMappingException("Value " + full + " is out of range for an integer ([" + Integer.MIN_VALUE + "," + Integer.MAX_VALUE + "])");
            }
            return (int) full;
        }
    ...
    /**
     * Parse a number, resolving hex and binary values, as well as a type suffix, and unsigned values
     *
     ...
     * @throws ObjectMappingException if unable to interpret an appropriate number from the input string.
     */
    static <T extends Number> T parseNumber(String input, final BiFunction<String, Integer, T> parseFunc, final BiFunction<String, Integer, T> unsignedParseFunc, final String suffix) throws ObjectMappingException {
        ...
        if (input.startsWith("0x", startIdx)) { // hex
            ...
            radix = 16;
            ++startIdx;
        } if (input.startsWith("0b", startIdx)) { // binary
            radix = 2;
            startIdx += 2;
        }
